function [SigmaCtllhInv, wt, Psit, zc] = calc_pv_trial_Scene(x, v, y, Z, sigmaU, sigmaV, phi)
    ...
    phi = (phi+rand(N,K)/100).*ZAct;            %** phi: [N x K],  0 if no dependency or chunk state is 0 in the trial,  phi if the shape is dependent and chunk is present in the trial
